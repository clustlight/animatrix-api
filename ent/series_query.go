...
func (sq *SeriesQuery) QuerySeasons() *SeasonQuery {
...
func (sq *SeriesQuery) Clone() *SeriesQuery {
	...
	return &SeriesQuery{
		...
		withSeasons: sq.withSeasons.Clone(),
...
func (sq *SeriesQuery) WithSeasons(opts ...func(*SeasonQuery)) *SeriesQuery {
	query := (&SeasonClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withSeasons = query
	return sq
}
...
func (sq *SeriesQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Series, error) {
	...
	if query := sq.withSeasons; query != nil {
		if err := sq.loadSeasons(ctx, query, nodes,
...
func (sq *SeriesQuery) loadSeasons(ctx context.Context, query *SeasonQuery, nodes []*Series, init func(*Series), assign func(*Series, *Season)) error {
	...
	for _, n := range neighbors {
		fk := n.series_seasons
		if fk == nil {
			return fmt.Errorf(`foreign-key "series_seasons" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "series_seasons" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
