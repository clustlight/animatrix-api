...
func (sq *SeasonQuery) QuerySeries() *SeriesQuery {
	query := (&SeriesClient{config: sq.config}).Query()
...
func (sq *SeasonQuery) Clone() *SeasonQuery {
	...
	return &SeasonQuery{
		...
		withSeries:   sq.withSeries.Clone(),
...
func (sq *SeasonQuery) WithSeries(opts ...func(*SeriesQuery)) *SeasonQuery {
	query := (&SeriesClient{config: sq.config}).Query()
...
func (sq *SeasonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Season, error) {
	...
	if query := sq.withSeries; query != nil {
		if err := sq.loadSeries(ctx, query, nodes, nil,
...
func (sq *SeasonQuery) loadSeries(ctx context.Context, query *SeriesQuery, nodes []*Season, init func(*Season), assign func(*Season, *Series)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Season)
	for i := range nodes {
		if nodes[i].series_seasons == nil {
			continue
		}
		fk := *nodes[i].series_seasons
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "series_seasons" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
